...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initPurchaseOrdersPage() {
        ...
        function renderTable(data) {
            tableBody.innerHTML = '';
            if (data.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="6" class="text-center">Không tìm thấy phiếu nhập nào.</td></tr>';
                return;
            }
    ...
    function initPurchaseEditPage() {
        ...
        function loadProductsIntoSelect() {
            ...
            if (!data || !data.products) {
                productSelect.innerHTML = '<option value="">Lỗi: Không tìm thấy bs_data</option>';
                return;
            }
            allProducts = data.products;
            productSelect.innerHTML = '<option value="">-- Chọn sách (Tìm theo tên hoặc SKU) --</option>';
            // Sắp xếp theo tên
        ...
        function renderItemsTable() {
            ...
            tempItems.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="text-end">
                        ${!isCompleted ? `<button class="btn btn-sm btn-outline-danger btn-delete-item" data-index="${index}">Xóa</button>` : ''}
                    </td>
                `;
        ...
        function savePurchaseOrder(status) {
            ...
            if (poId) { // Chế độ Sửa
                allPurchases = allPurchases.map(p => p.id === parseInt(poId) ? poData : p);
            } else { // Chế độ Tạo mới
            ...
            alert(`Đã ${status === 'completed' ? 'Hoàn thành' : 'Lưu tạm'} phiếu nhập!`);
        ...
        function updateStockAndCost() {
            ...
            tempItems.forEach(item => {
                ...
                if (productIndex > -1) {
                    // Cập nhật số lượng
                    data.products[productIndex].qty = (data.products[productIndex].qty || 0) + item.quantity;
        ...
        function disableForm() {
            ...
            statusDisplay.innerHTML = '<span class="status delivered d-block w-100">Đã hoàn thành</span>';
            addItemCard.style.display = 'none'; // Ẩn form thêm SP
            actionButtons.innerHTML = '<p class="text-success text-center fw-bold">Phiếu đã hoàn thành, không thể sửa.</p>'; // Xóa các nút
        ...
        addBtn.addEventListener('click', function() {
            ...
            if (existingItem) {
                alert('Sản phẩm này đã có trong phiếu. Bạn có thể xóa và thêm lại với số lượng mới.');
                return;
            }
        ...
        if (poId) { // Chế độ Sửa/Xem
            ...
            if (po) {
                ...
                if (po.status === 'completed') {
                    ...
                    statusDisplay.innerHTML = '<span class="status pending d-block w-100">Chưa hoàn thành</span>';
                }
            } else {
   ...
   function initOrdersPage() {
        ...
        function renderTable(filteredOrders) {
            ...
            filteredOrders.forEach(order => {
                ...
                row.innerHTML = `
                    <td>#${order.id}</td>
                    <td>${order.shippingAddress.name || `User: ${order.userId}`}</td>
