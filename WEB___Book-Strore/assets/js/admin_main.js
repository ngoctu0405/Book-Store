...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initProductsPage() {
    ...
    products.forEach((p) => {
      ...
      const toggleButtonClass =
        p.status === "visible" ? "btn-outline-danger" : "btn-outline-success";
  ...
  function initProductEditPage() {
    ...
    if (!preview) {
      ...
      preview.style.cssText =
        "max-width: 200px; max-height: 200px; margin-top: 15px; display: none;";
  ...
  function initPurchaseEditPage() {
    ...
    if (!addButton || !productsTableBody || !totalCell || !productSelect)
      return;
    ...
    addButton.addEventListener("click", function () {
      ...
      const cost =
        parseInt(document.getElementById("purchase-cost").value) || 0; // Cần <input id="purchase-cost">
      const quantity =
        parseInt(document.getElementById("purchase-quantity").value) || 1; // Cần <input id="purchase-quantity">
  ...
  function initOrderDetailPage() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (oldStatus !== "Đã giao" && newStatus === "Đã giao") {
        ...
        oldStatus === "Đã giao" &&
        (newStatus === "Đã hủy" || newStatus === "Chờ xử lý")
      ) {
        let data = db_get("bs_data");
        let products = data.products || [];
        order.items.forEach((item) => {
