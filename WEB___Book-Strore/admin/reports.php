    ...
    <script>
            ...
            if (order.items && Array.isArray(order.items)) {
              order.items.forEach((item) => {
                transactions.push({
                  ...
                  qty: item.quantity || 0,
                  date:
                    order.orderDate || order.date || new Date().toISOString(),
                });
              });
            }
          }
      ...
      function displayStockAlerts() {
        ...
        if (outOfStock.length === 0) {
          outOfStockList.innerHTML =
            '<p class="text-muted mb-0">✓ Không có sản phẩm hết hàng</p>';
        } else {
        ...
        if (lowStock.length === 0) {
          lowStockList.innerHTML =
            '<p class="text-muted mb-0">✓ Không có sản phẩm sắp hết</p>';
        } else {
      ...
      function renderReport(reportData, start, end) {
        ...
        if (reportData.length === 0) {
          html =
            '<tr><td colspan="6" class="text-center text-muted">Không có dữ liệu sản phẩm để hiển thị.</td></tr>';
        } else {
